...
/// </summary>
public class TenorthEventHandler : MonoBehaviour
{
    ...
    public static GameObject MoritzTenorth;
    ...
    string URL = "https://query.wikidata.org/sparql?query=SELECT ?personLabel ?personDescription ?employerLabel (GROUP_CONCAT(DISTINCT ?itemLabel; separator= ', ') as ?workLabel)   WHERE {?person wdt:P106 ?description. ?person wdt:P106 wd:Q15976092. ?person wdt:P18 <http://commons.wikimedia.org/wiki/Special:FilePath/MoritzTenorth.jpg>. ?person wdt:P108 ?employer. ?person wdt:P800 ?item.SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. ?person rdfs:label ?personLabel. ?person schema:description ?personDescription. ?employer rdfs:label ?employerLabel. ?item rdfs:label ?itemLabel.}.} GROUP BY ?personLabel ?personDescription ?employerLabel";
 ...
 if (StatusFilter == TrackingStatusFilter.Tracked_ExtendedTracked && status == Status.EXTENDED_TRACKED)
    ...
    public IEnumerator getData(string uri)
    {
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                MoritzTenorth = GameObject.Find("Moritz Tenorth");

                ScanScript.Text();

                FoundScript.SetParent(MoritzTenorth);
                FoundScript.Text();
                
                NameScript.SetParent(MoritzTenorth);
                NameScript.Text(intresponse);

                DescriptionScript.SetParent(MoritzTenorth);
                DescriptionScript.Text(intresponse);

                EmployerScript.SetParent(MoritzTenorth);
                EmployerScript.Text(intresponse);

                WorkScript.SetParent(MoritzTenorth);
    ...
    private void processResponse(string responsetext)
    {
        ...
        if (responsetext.Contains("Moritz Tenorth"))
        {
