...
/// </summary>
public class PepperEventHandler : MonoBehaviour
{
...
string URL = "https://query.wikidata.org/sparql?query=SELECT ?personLabel ?personDescription (GROUP_CONCAT(DISTINCT ?manufacturerLabel; separator= ', ') as ?manufacturers) WHERE {?person wdt:P176 ?manufacturer. ?person wdt:P18 <http://commons.wikimedia.org/wiki/Special:FilePath/Kalasataman%20terveys-%20ja%20hyvinvointikeskus%20-%20Helsinki%20-%204.jpg>. SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. ?person rdfs:label ?personLabel. ?person schema:description ?personDescription. ?manufacturer rdfs:label ?manufacturerLabel. } .} GROUP BY ?personLabel ?personDescription";  
  protected virtual void Start()
    ...
    public IEnumerator getData(string uri)
    {
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                FoundScript.SetParent(Pepper);
                FoundScript.Text();

                NameScript.SetParent(Pepper);
                NameScript.Text(intresponse);

                DescriptionScript.SetParent(Pepper);
                DescriptionScript.Text(intresponse);

                EmployerScript.SetParent(Pepper);
                EmployerScript.Text(intresponse);

                WorkScript.SetParent(Pepper);
                WorkScript.noText();
    ...
    private void processResponse(string responsetext)
    {
        ...
        if (responsetext.Contains("Pepper"))
        {
