...
/// </summary>
public class TuringEventHandler : MonoBehaviour
{
    ...
    public static GameObject AlanTuring;
...
string URL = "https://query.wikidata.org/sparql?query=SELECT ?personLabel ?personDescription ?employerLabel (GROUP_CONCAT(DISTINCT ?itemLabel; separator= ', ') as ?workLabel)   WHERE {?person wdt:P18 <http://commons.wikimedia.org/wiki/Special:FilePath/Alan%20Turing%20Aged%2016.jpg>. ?person wdt:P108 ?employer. ?person wdt:P800 ?item. ?person p:P108 ?employerstatement. ?employerstatement ps:P108 ?employer. ?employerstatement pq:P580 ?starttime. FILTER NOT EXISTS{?person p:P108/pq:P580 ?starttime_. FILTER (?starttime_ > ?starttime).} SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. ?person rdfs:label ?personLabel. ?person schema:description ?personDescription. ?employer rdfs:label ?employerLabel. ?item rdfs:label ?itemLabel.} }GROUP BY ?personLabel ?personDescription ?employerLabel";
 ...
 if (StatusFilter == TrackingStatusFilter.Tracked_ExtendedTracked && status == Status.EXTENDED_TRACKED)
    ...
    public IEnumerator getData(string uri)
    {
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                AlanTuring = GameObject.Find("Alan Turing");

                ScanScript.Text();

                FoundScript.SetParent(AlanTuring);
                FoundScript.Text();
                
                NameScript.SetParent(AlanTuring);
                NameScript.Text(intresponse);

                DescriptionScript.SetParent(AlanTuring);
                DescriptionScript.Text(intresponse);

                EmployerScript.SetParent(AlanTuring);
                EmployerScript.Text(intresponse);

                WorkScript.SetParent(AlanTuring);
    ...
    private void processResponse(string responsetext)
    {
        ...
        if (responsetext.Contains("Alan Turing"))
        {
