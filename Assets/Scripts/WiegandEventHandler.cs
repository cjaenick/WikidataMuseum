...
/// </summary>
public class WiegandEventHandler : MonoBehaviour
{
    ...
    public static GameObject ThomasWiegand;
    ...
    string URL = "https://query.wikidata.org/sparql?query=SELECT ?personLabel ?personDescription ?employerLabel ?workLabel WHERE { ?person wdt:P106 wd:Q15976092. ?person wdt:P18 <http://commons.wikimedia.org/wiki/Special:FilePath/AI%20for%20Good%20Global%20Summit%202018%20%2841233458815%29.jpg>. ?person wdt:P800 ?work. ?person wdt:P108 ?employer.SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. }.}"; 
...
if (StatusFilter == TrackingStatusFilter.Tracked_ExtendedTracked && status == Status.EXTENDED_TRACKED)
    ...
    public IEnumerator getData(string uri)
    {
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                ThomasWiegand = GameObject.Find("Thomas Wiegand");

                ScanScript.Text();

                FoundScript.SetParent(ThomasWiegand);
                FoundScript.Text();
                
                NameScript.SetParent(ThomasWiegand);
                NameScript.Text(intresponse);

                DescriptionScript.SetParent(ThomasWiegand);
                DescriptionScript.Text(intresponse);

                EmployerScript.SetParent(ThomasWiegand);
                EmployerScript.Text(intresponse);

                WorkScript.SetParent(ThomasWiegand);
    ...
    private void processResponse(string responsetext)
    {
        ...
        if (responsetext.Contains("Thomas Wiegand"))
        {
