...
/// </summary>
public class NorvigEventHandler : MonoBehaviour
{
    ...
    public static GameObject PeterNorvig;
...
string URL = "https://query.wikidata.org/sparql?query=SELECT ?personLabel ?personDescription ?employerLabel ?workLabel WHERE {?person wdt:P106 wd:Q15976092. ?person wdt:P800 ?work. ?person wdt:P18 <http://commons.wikimedia.org/wiki/Special:FilePath/Peter%20Norvig%20in%202019.jpg>. ?person p:P108 ?employerstatement. ?employerstatement ps:P108 ?employer. ?employerstatement pq:P580 ?starttime. FILTER NOT EXISTS{?person p:P108/pq:P580 ?starttime_. FILTER (?starttime_ > ?starttime).} SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. }. }";  
  protected virtual void Start()
    ...
    public IEnumerator getData(string uri)
    {
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                PeterNorvig = GameObject.Find("Peter Norvig");

                ScanScript.Text();
                
                FoundScript.SetParent(PeterNorvig);
                FoundScript.Text();

                NameScript.SetParent(PeterNorvig);
                NameScript.Text(intresponse);

                DescriptionScript.SetParent(PeterNorvig);
                DescriptionScript.Text(intresponse);

                EmployerScript.SetParent(PeterNorvig);
                EmployerScript.Text(intresponse);

                WorkScript.SetParent(PeterNorvig);
    ...
    private void processResponse(string responsetext)
    {
        ...
        if (responsetext.Contains("Peter Norvig"))
        {
